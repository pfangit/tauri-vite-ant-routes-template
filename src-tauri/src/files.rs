...
pub fn list_directories_internal<P: AsRef<Path>>(dir: P) -> io::Result<FileOrDir> {
    let path = dir.as_ref();
    let name = path
        .file_name()
        .unwrap_or_default()
    ...
    if is_directory {
        for entry in fs::read_dir(path)? {
            ...
            if entry_path.is_dir() {
                children.push(list_directories_internal(entry_path)?);
            }
        }
    }
...
#[tauri::command]
pub fn list_directories(dir_path: &str) -> Result<FileOrDir, String> {
    list_directories_internal(dir_path).map_err(|e| e.to_string())
}
